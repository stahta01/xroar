...
// Set if keys should be intepreted based on their (potentially modified)
...
/* Track which unicode value was last generated by key presses. */
static int unicode_last_scancode[SDL_NUM_SCANCODES];
...
static void emulator_command(int cmdkey, _Bool shift) {
	switch (cmdkey) {
...
static void control_keypress(SDL_Keysym *keysym) {
...
void sdl_keyrelease(SDL_Keysym *keysym) {
	...
	// If scancode has priority, never do a unicode lookup.
	if (scancode_priority[scancode]) {
		keyboard_release(xroar_keyboard_interface, scancode_to_dkey[scancode]);
		return;
	}
