...
void sdl_js_device_added(int index) {
	...
	if (index >= num_devices) {
		...
		if (device_id)
			SDL_free(device_id);
		device_id = SDL_GetJoysticks(&new_num_devices);
	...
	if (SDL_IsGamepad(index)) {
		gamecontroller = SDL_OpenGamepad(device_id[index]);
		if (!gamecontroller)
			return;
	} else {
		joystick = SDL_OpenJoystick(device_id[index]);
		if (!joystick)
			return;
	}
	...
	if (d->is_gamecontroller) {
		d->num_axes = SDL_GAMEPAD_AXIS_COUNT;
		d->num_buttons = SDL_GAMEPAD_BUTTON_COUNT;
		guid = SDL_GetGamepadGUIDForID(device_id[index]);
	} else {
		d->num_axes = SDL_GetNumJoystickAxes(joystick);
		d->num_buttons = SDL_GetNumJoystickButtons(joystick);
		guid = SDL_GetJoystickGUIDForID(device_id[index]);
	}
	...
	if (gamecontroller) {
		joy_name = SDL_GetGamepadName(gamecontroller);
	} else {
		joy_name = SDL_GetJoystickName(joystick);
	}
	if (!joy_name) {
		joy_name = "Joystick";
	}
	...
	if (logging.level > 1) {
		...
		if (d->is_gamecontroller) {
			vendor_id = SDL_GetGamepadVendor(gamecontroller);
			product_id = SDL_GetGamepadProduct(gamecontroller);
			product_version = SDL_GetGamepadProductVersion(gamecontroller);
		} else {
			vendor_id = SDL_GetJoystickVendor(joystick);
			product_id = SDL_GetJoystickProduct(joystick);
			product_version = SDL_GetJoystickProductVersion(joystick);
		}
		LOG_PRINT("\tGUID: %s\n", guid_str);
		if (vendor_id) {
	...
	if (d->is_gamecontroller) {
		...
		sdsfree(tmp);
		//
		tmp = sdscatprintf(sdsempty(), "physical:%d,%d", index, SDL_GAMEPAD_AXIS_LEFTX);
		jc0->axis_specs[0] = xstrdup(tmp);
		sdsfree(tmp);
		tmp = sdscatprintf(sdsempty(), "physical:%d,%d", index, SDL_GAMEPAD_AXIS_LEFTY);
		jc0->axis_specs[1] = xstrdup(tmp);
		sdsfree(tmp);
		//
		tmp = sdscatprintf(sdsempty(), "physical:%d,%%%d", index, (1 << SDL_GAMEPAD_BUTTON_SOUTH) | (1 << SDL_GAMEPAD_BUTTON_LEFT_SHOULDER));
		jc0->button_specs[0] = xstrdup(tmp);
		sdsfree(tmp);
		tmp = sdscatprintf(sdsempty(), "physical:%d,%%%d", index, (1 << SDL_GAMEPAD_BUTTON_EAST));
		...
		sdsfree(tmp);
		//
		tmp = sdscatprintf(sdsempty(), "physical:%d,%d", index, SDL_GAMEPAD_AXIS_RIGHTX);
		jc1->axis_specs[0] = xstrdup(tmp);
		sdsfree(tmp);
		tmp = sdscatprintf(sdsempty(), "physical:%d,%d", index, SDL_GAMEPAD_AXIS_RIGHTY);
		jc1->axis_specs[1] = xstrdup(tmp);
		sdsfree(tmp);
		//
		tmp = sdscatprintf(sdsempty(), "physical:%d,%%%d", index, (1 << SDL_GAMEPAD_BUTTON_WEST) | (1 << SDL_GAMEPAD_BUTTON_RIGHT_SHOULDER));
		jc1->button_specs[0] = xstrdup(tmp);
		sdsfree(tmp);
		tmp = sdscatprintf(sdsempty(), "physical:%d,%%%d", index, (1 << SDL_GAMEPAD_BUTTON_NORTH));
...
void sdl_js_device_removed(int index) {
	...
	if (d->is_gamecontroller) {
		SDL_CloseGamepad(d->handle.gamecontroller);
		d->handle.gamecontroller = NULL;
	} else {
		SDL_CloseJoystick(d->handle.joystick);
...
static void sdl_js_physical_init(void) {
	...
	num_devices = 0;
	device_id = SDL_GetJoysticks(&num_devices);
...
static struct sdl_js_device *open_device(int joystick_index) {
	...
	if (d->is_gamecontroller) {
		d->handle.gamecontroller = SDL_OpenGamepad(joystick_index);
	...
	d->handle.joystick = SDL_OpenJoystick(joystick_index);
...
static void debug_controls(struct sdl_js_device *d) {
	...
	for (int i = 0; i < d->num_axes; ++i) {
		unsigned v;
		if (d->is_gamecontroller) {
			v = SDL_GetGamepadAxis(d->handle.gamecontroller, i) + 32768;
		} else {
			v = SDL_GetJoystickAxis(d->handle.joystick, i) + 32768;
		}
		if (d->debug_axes[i] != v) {
			report = 1;
			d->debug_axes[i] = v;
	...
	}
	{
		...
		for (int i = 0; i < d->num_buttons; ++i) {
			...
			if (d->is_gamecontroller) {
				...
				b = SDL_GetJoystickButton(d->handle.joystick, i);
			}
			v |= (b << i);
		}
...
static int sdl_js_axis_read(void *sptr) {
	...
	unsigned ret;
	if (c->device->is_gamecontroller) {
		ret = SDL_GetGamepadAxis(c->device->handle.gamecontroller, c->control.axis) + 32768;
	} else {
		ret = SDL_GetJoystickAxis(c->device->handle.joystick, c->control.axis) + 32768;
	}
	if (c->inverted)
		ret ^= 0xffff;
	return ret;
...
static int sdl_js_button_read(void *sptr) {
	...
	for (int i = 0; i < c->device->num_buttons; ++i) {
		...
		if (c->device->is_gamecontroller) {
			...
			b = SDL_GetJoystickButton(c->device->handle.joystick, i);
		}
		v |= (b << i);
	}
