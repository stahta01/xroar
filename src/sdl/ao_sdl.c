...
static void *new(void *cfg) {
	...
	struct ao_sdl_interface *aosdl = xmalloc(sizeof(*aosdl));
	*aosdl = (struct ao_sdl_interface){0};
	...
	if (SDL_OpenAudio(&desired, &aosdl->audiospec) < 0) {
...
static void ao_sdl_free(void *sptr) {
	struct ao_sdl_interface *aosdl = sptr;
...
static void callback(void *userdata, Uint8 *stream, int len) {
	struct ao_sdl_interface *aosdl = userdata;
	(void)len;  /* unused */
